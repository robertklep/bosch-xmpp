...
class EasyControlClient extends BaseClient {
  ...
  generateEncryptionKey() {
    const password = this.opts.password.length <= 8 ? this.opts.password : Buffer.from(this.opts.password).toString('base64');
    const hash1    = MD5(Buffer.concat([ Buffer.from(this.opts.accessKey), MAGIC ]));
    const hash2    = MD5(Buffer.concat([ MAGIC, Buffer.from(password) ]));
